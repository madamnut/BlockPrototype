...
public static class VoxelFluidMesher
{
    ...
    {
        if (terrain.GetBlock(worldX, worldY, worldZ) != VoxelBlockType.Air)
        {
            return 1f;
        }
    ...
    {
        return terrain.GetBlock(worldX, worldY + 1, worldZ) != VoxelBlockType.Air || terrain.GetFluid(worldX, worldY + 1, worldZ).Exists;
    }
