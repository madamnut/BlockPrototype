...
public sealed class VoxelWorldRuntime : MonoBehaviour
{
    ...
    private ChunkColumnInstance CreateChunkColumnInstance()
    {
        ...
        for (int subChunkY = 0; subChunkY < VoxelTerrainData.SubChunkCountY; subChunkY++)
        {
            column.SolidSubChunks[subChunkY] = CreateSubChunkInstance(rootObject.transform, subChunkY, worldMaterial, true);
            column.FluidSubChunks[subChunkY] = CreateSubChunkInstance(rootObject.transform, subChunkY, fluidMaterial, false);
        }
    ...
    private SubChunkInstance CreateSubChunkInstance(Transform parent, int subChunkY, Material material, bool createCollider)
    {
        ...
        meshRenderer.shadowCastingMode = createCollider ? ShadowCastingMode.On : ShadowCastingMode.Off;
        meshRenderer.receiveShadows = createCollider;
    ...
    private static bool HasAnyVisibleGeometry(ChunkColumnInstance column)
    {
        ...
        {
            if (IsSubChunkVisible(column.SolidSubChunks[i]) || IsSubChunkVisible(column.FluidSubChunks[i]))
            {
                return true;
            }
        }
