...
public sealed class TerraContinentalnessOffsetGraphWindow : EditorWindow
{
    private const float PointHandleRadius = 6f;
    private const float TopFieldLabelWidth = 140f;
    private const float TopFieldInputWidth = 110f;
    ...
    private TerraContinentalnessOffsetGraphAsset _asset;
    private SerializedObject _serializedObject;
    private SerializedProperty _pointsProperty;
    private int _draggingPointIndex = -1;

    public static void Open(TerraContinentalnessOffsetGraphAsset asset)
    {
        TerraContinentalnessOffsetGraphWindow window = GetWindow<TerraContinentalnessOffsetGraphWindow>("Terra Cont Graph");
    ...
    {
        if (_asset == null && Selection.activeObject is TerraContinentalnessOffsetGraphAsset selectedAsset)
        {
    ...
    {
        if (Selection.activeObject is TerraContinentalnessOffsetGraphAsset selectedAsset)
        {
            SetAsset(selectedAsset);
            Repaint();
        }
    ...
    private void OnGUI()
    {
        ...
        {
            EditorGUILayout.LabelField("Terra Continentalness Offset Graph", EditorStyles.boldLabel);
        ...
        if (_asset == null)
        {
            EditorGUILayout.HelpBox("Select a TerraContinentalnessOffsetGraph asset or open the window from the asset inspector.", MessageType.Info);
            return;
        }
    ...
    private void SetAsset(TerraContinentalnessOffsetGraphAsset asset)
    {
        _asset = asset;
    ...
    private void DrawGraph()
    {
        ...
        float minY = GetMinPointY() - 10f;
        float maxY = GetMaxPointY() + 10f;
    ...
    private void HandleGraphInput(Rect graphRect, float minY, float maxY)
    {
        ...
        switch (current.GetTypeForControl(controlId))
        {
            ...
            case EventType.MouseDrag:
                ...
                yProperty.floatValue = FromGraphY(graphRect, current.mousePosition.y, minY, maxY);
    ...
    private void DrawPointList()
    {
        ...
        for (int i = 0; i < _pointsProperty.arraySize; i++)
        {
            ...
            yProperty.floatValue = EditorGUILayout.FloatField(yProperty.floatValue, GUILayout.Width(90f));
            GUILayout.FlexibleSpace();
        ...
        if (GUILayout.Button("Add Point", GUILayout.Width(100f)))
        {
            ...
            newPoint.FindPropertyRelative("y").floatValue = 0f;
        }
        EditorGUILayout.EndHorizontal();
    }
