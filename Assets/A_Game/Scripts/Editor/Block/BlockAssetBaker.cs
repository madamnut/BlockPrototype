...
public static class BlockAssetBaker
{
    ...
    public static void Bake(BlockAuthoringDatabase authoringDatabase)
    {
        if (authoringDatabase == null)
        {
            Debug.LogError("Block bake failed because the authoring database is null.");
            return;
        }

        string textureRootPath = AssetDatabase.GetAssetPath(authoringDatabase.TextureRootFolder);
        BlockAuthoringEntry[] entries = authoringDatabase.Blocks;
        if (entries == null || entries.Length == 0)
        {
            Debug.LogError("Block bake requires at least one block entry.", authoringDatabase);
            return;
        }
        ...
        {
            Debug.LogError("Block bake needs a valid texture root folder containing textures that match the block name and layout naming rules.", authoringDatabase);
            return;
        }

        Dictionary<string, ushort> layerByAssetPath = new(StringComparer.OrdinalIgnoreCase);
        List<Texture2D> layeredTextures = new();
        List<BlockDefinition> bakedDefinitions = new(entries.Length);

        for (int i = 0; i < entries.Length; i++)
        {
            BlockAuthoringEntry entry = entries[i];
            if (string.IsNullOrWhiteSpace(entry.BlockName))
            {
                Debug.LogError($"Block bake found an empty block name for block id {entry.BlockId}.", authoringDatabase);
                return;
            }
            ...
            bakedDefinitions.Add(new BlockDefinition(
                entry.BlockId,
                entry.BlockName,
                entry.Kind,
                layers[0],
        ...
        {
            Debug.LogError("Block bake produced no texture layers.", authoringDatabase);
            return;
        }
        ...
        BlockDatabase runtimeDatabase = authoringDatabase.RuntimeDatabase;
        if (runtimeDatabase == null)
        {
            string databasePath = $"{outputFolder}/{authoringDatabase.name}_RuntimeDatabase.asset";
            runtimeDatabase = AssetDatabase.LoadAssetAtPath<BlockDatabase>(databasePath);
            if (runtimeDatabase == null)
            {
                runtimeDatabase = ScriptableObject.CreateInstance<BlockDatabase>();
        ...
        Debug.Log(
            $"Block bake completed. Blocks: {bakedDefinitions.Count}, Unique textures: {layeredTextures.Count}, TextureArray: {textureArrayPath}",
            authoringDatabase);
    }
    ...
    private static Texture2D FindFaceTexture(Texture2D[] textures, BlockAuthoringEntry entry, BlockFace face, Object context)
    {
        string[] expectedTokens = GetExpectedTextureTokens(entry, face, textures);
        if (expectedTokens.Length == 0)
        {
            Debug.LogError(
                entry.Kind == ContentKind.Foliage
                    ? $"Foliage bake could not find '{entry.BlockName}'."
                    : $"Block bake could not infer a texture layout for block '{entry.BlockName}'. Supported names are '{entry.BlockName}_All', '{entry.BlockName}_Top/_Bottom/_Side', '{entry.BlockName}_TopBottom/_Side', '{entry.BlockName}_Top/_Side', or full '{entry.BlockName}_Top/_Bottom/_Front/_Back/_Left/_Right'.",
                context);
            return null;
        }
        ...
        {
            Debug.LogError(
                $"Block bake could not find a texture for block '{entry.BlockName}' face '{face}'. Expected one of: {string.Join(", ", expectedTokens)}.",
                context);
            return null;
        }
        ...
        {
            Debug.LogError(
                $"Block bake found multiple textures for block '{entry.BlockName}' face '{face}'. Please keep only one matching texture.",
                context);
            return null;
        }
    ...
    private static string[] GetExpectedTextureTokens(BlockAuthoringEntry entry, BlockFace face, Texture2D[] textures)
    {
        ...
        if (entry.Kind == ContentKind.Foliage)
        {
            bool hasFoliage = HasTexture(textures, blockToken, string.Empty);
            return hasFoliage
                ? new[] { blockToken }
                : Array.Empty<string>();
        }
        ...
        if (hasIndividual)
        {
            return face switch
            {
                BlockFace.Top => new[] { blockToken + "top" },
                BlockFace.Bottom => new[] { blockToken + "bottom" },
                BlockFace.Left => new[] { blockToken + "left" },
                BlockFace.Right => new[] { blockToken + "right" },
                BlockFace.Back => new[] { blockToken + "back" },
        ...
        bool hasTopBottomShared =
            (HasTexture(textures, blockToken, "topbottom") || HasTexture(textures, blockToken, "bottomtop")) &&
            HasTexture(textures, blockToken, "side");

        if (hasTopBottomShared)
        {
            return face switch
            {
                BlockFace.Top => new[] { blockToken + "topbottom", blockToken + "bottomtop" },
                BlockFace.Bottom => new[] { blockToken + "topbottom", blockToken + "bottomtop" },
                _ => new[] { blockToken + "side" },
            };
        }
    ...
    private static Texture2DArray BuildTextureArray(List<Texture2D> sourceTextures, Object context)
    {
        ...
        for (int i = 0; i < sourceTextures.Count; i++)
        {
            ...
            {
                Debug.LogError(
                    $"Block bake requires all textures to have the same size. '{sourceTextures[i].name}' is {sourceTextures[i].width}x{sourceTextures[i].height}, expected {width}x{height}.",
                    context);
                return null;
            }
        }
    ...
    {
        string sanitizedPath = string.IsNullOrWhiteSpace(requestedPath) ? "Assets/A_Game/Generated/Block" : requestedPath.Trim();
    ...
    private static bool ValidateTargetMaterial(Material targetMaterial, Object context)
    {
        ...
        Debug.LogError(
            $"Assigned target material '{targetMaterial.name}' uses shader '{shaderName}', but block baking requires a material using 'YD/Voxel Texture Array Lit' or another shader with a _BlockTextures Texture2DArray property.",
            context);
        return false;
    }
}
