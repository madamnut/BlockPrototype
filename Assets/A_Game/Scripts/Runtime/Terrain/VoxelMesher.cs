...
public static class VoxelMesher
{
                ...
                while (GetAxis(x, d) < VoxelTerrainData.SubChunkSize)
                {
                    ...
                    for (int vCoord = 0; vCoord < VoxelTerrainData.SubChunkSize; vCoord++)
                    {
                        ...
                        for (int uCoord = 0; uCoord < VoxelTerrainData.SubChunkSize; uCoord++)
                        {
                            ...
                            if (TryBuildMaskCell(a, b, d, faceTextureLookup, out JobMaskCell maskCell))
                            {
                                mask[n] = maskCell;
                            }
                            else
        ...
        for (int d = 0; d < 3; d++)
        {
            ...
            for (x[d] = -1; x[d] < dims[d];)
            {
                ...
                for (x[v] = 0; x[v] < dims[v]; x[v]++)
                {
                    ...
                    {
                        BlockType a = SampleBlock(terrain, chunkX, subChunkY, chunkZ, x[0], x[1], x[2]);
                        BlockType b = SampleBlock(terrain, chunkX, subChunkY, chunkZ, x[0] + q[0], x[1] + q[1], x[2] + q[2]);

                        if (TryBuildMaskCell(a, b, d, blockDatabase, out MaskCell maskCell))
                        {
                            mask[n] = maskCell;
                        }
                        else
    ...
    private static ushort GetFaceTextureLayer(NativeArray<ushort> faceTextureLookup, ushort blockId, int axis, bool backFace)
    {
        ...
        {
            0 => backFace ? (int)BlockFace.Left : (int)BlockFace.Right,
            1 => backFace ? (int)BlockFace.Bottom : (int)BlockFace.Top,
            2 => backFace ? (int)BlockFace.Back : (int)BlockFace.Front,
            _ => (int)BlockFace.Front,
        };
    ...
    private static bool TryBuildMaskCell(ushort a, ushort b, int axis, NativeArray<ushort> faceTextureLookup, out JobMaskCell maskCell)
    {
        if (!TryResolveVisibleFace(a, b, out ushort visibleBlockId, out bool backFace))
        {
            maskCell = default;
            return false;
        }

        maskCell = new JobMaskCell
        {
            Exists = 1,
            TextureLayer = GetFaceTextureLayer(faceTextureLookup, visibleBlockId, axis, backFace),
            BackFace = backFace ? (byte)1 : (byte)0,
        };
        return true;
    }

    private static bool TryBuildMaskCell(BlockType a, BlockType b, int axis, BlockDatabase blockDatabase, out MaskCell maskCell)
    {
        if (!TryResolveVisibleFace((ushort)a, (ushort)b, out ushort visibleBlockId, out bool backFace))
        {
            maskCell = default;
            return false;
        }

        maskCell = new MaskCell
        {
            Exists = true,
            TextureLayer = blockDatabase.GetFaceTextureLayer(visibleBlockId, GetFace(axis, backFace)),
            BackFace = backFace,
        };
        return true;
    }

    private static bool TryResolveVisibleFace(ushort a, ushort b, out ushort visibleBlockId, out bool backFace)
    {
        bool aExists = a != 0;
        bool bExists = b != 0;
        ...
        bool aIsLeaves = a == LeavesBlockId;
        bool bIsLeaves = b == LeavesBlockId;
        if (aIsLeaves == bIsLeaves)
        {
            visibleBlockId = 0;
            backFace = false;
            return false;
    ...
    private static BlockFace GetFace(int axis, bool backFace)
    {
        return axis switch
        {
            0 => backFace ? BlockFace.Left : BlockFace.Right,
            1 => backFace ? BlockFace.Bottom : BlockFace.Top,
            2 => backFace ? BlockFace.Back : BlockFace.Front,
            _ => BlockFace.Front,
        };
    }
