...
[CreateAssetMenu(
    fileName = "CurveLut",
    menuName = "World/WorldGen/Curve LUT")]
public class CurveLutAsset : ScriptableObject
{
    [System.Serializable]
    ...
    [SerializeField] private float outputMin = -1f;
    [SerializeField] private float outputMax = TerrainData.WorldHeight - 1;
    ...
    public float OutputMin => Mathf.Min(outputMin, outputMax);
    public float OutputMax => Mathf.Max(outputMin, outputMax);
    ...
    {
        return Mathf.Clamp(curve.Evaluate(input), OutputMin, OutputMax);
    }
    ...
    private void NormalizeFixedPoints()
    {
        ...
        for (int i = 0; i < fixedPoints.Count; i++)
        {
            ...
            fixedPoint.output = Mathf.Clamp(fixedPoint.output, min, max);
    ...
    private static int FindKeyIndex(List<Keyframe> keys, float input)
    {
        ...
        return nearestDistance <= nearestMatchRange ? nearestIndex : -1;
    }
}
